...
# 
# ---------------------------------------------------------------------------------------------------
def astar_main_run(filename, printTour = False):
    ...
    print("------------------------------------------")
    print("Astar Algorithm -", filename)
    start_time = time.time()

    computePathastar(number_of_nodes, TOUR_START_NODE_INDEX, costs, printTour)
...
# ---------------------------------------------------------------------------------------------------
# computePathastar() method produces the tour using astar algorithm
# ---------------------------------------------------------------------------------------------------
def computePathastar(number_of_nodes, start_node, costs, printTour):
    global TOUR_START_NODE_INDEX
    visited_nodes = []
        ...
        for key, value in iter(costs.items()):
            if key[0] == start_node and key[0] != key[1] and key[1] not in visited_nodes:
                if(key[0]==start):
                    ...
                    heuristic = costs[start,key[0]]
                node_distances[key[1]] = value+heuristic
                total_length[key[1]] = value
            elif key[1] == start_node and key[0] != key[1] and key[0] not in visited_nodes:
                if(key[1]==start):
                    heuristic = 0
                else:
                    heuristic = costs[start,key[1]]
                node_distances[key[0]] = value+heuristic
                total_length[key[0]] = value
        start_node = min(node_distances, key=node_distances.get)
        total_tour_length += total_length.pop(start_node)
    visited_nodes.append(start_node)
